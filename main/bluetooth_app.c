...
#define ALARM_TRIGGER_VALUE 0xAA
#define ALARM_ON_COMMAND 0xBB
...
ESP_BLE_MESH_MODEL_PUB_DEFINE(pub_1, 5, ROLE_NODE);
...
static esp_ble_mesh_deinit_param_t deinit_param = {
    .erase_flash = true};
                                       ...
                                       esp_ble_mesh_cfg_server_cb_param_t *param)
{
    if (event == ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT)
    {
        ...
        case ESP_BLE_MESH_MODEL_OP_MODEL_SUB_ADD:
            ESP_LOGI(TAG, "Subscription ok");

            xEventGroupSetBits(     // Encerra o provisionamento
                xCreatedEventGroup, // The event group being updated.
                PROV_END);          // The bits being set.
            break;

        case ESP_BLE_MESH_MODEL_OP_MODEL_PUB_SET:
            ESP_LOGI(TAG, "Publication ok");
            break;
        default:
            break;
        }
                                      ...
                                      esp_ble_mesh_model_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_MODEL_OPERATION_EVT:
        if (param->model_operation.opcode == ESP_BLE_MESH_VND_MODEL_OP_SEND)
        {
            ...
            ESP_LOGI(TAG, "Recv Msg %u, tid 0x%04x", *param->model_operation.msg, tid);
            /*
        ...
        if(*param->model_operation.msg == ALARM_ON_COMMAND){
...
{
    esp_err_t err;
    uint8_t data = ALARM_TRIGGER_VALUE;

    err = esp_ble_mesh_model_publish(&vnd_models[0], ESP_BLE_MESH_VND_MODEL_OP_STATUS, sizeof(data),
                                     (uint8_t *)&data, ROLE_NODE);
    if (err)
    {
        ESP_LOGE(TAG, "Failed to send message 0x%06x", ESP_BLE_MESH_VND_MODEL_OP_STATUS);
    }
...
esp_err_t ble_mesh_reset(void)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "Ble deinit error");
        return err;
    }
    return ble_mesh_init();
}
